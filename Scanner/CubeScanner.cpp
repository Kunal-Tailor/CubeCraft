...
RubiksCube::COLOR CubeScanner::classifyColor(const Vec3b& bgr) {
    ...
    if (s < 60 && v > 150) return RubiksCube::COLOR::WHITE;

    // Yellow: hue 20-35, decent saturation
    if (h >= 20 && h <= 35 && s > 80 && v > 100) return RubiksCube::COLOR::YELLOW;

    // Orange: hue 5-20, high saturation
    if (h >= 5 && h <= 20 && s > 100 && v > 100) return RubiksCube::COLOR::ORANGE;

    // Red: hue wraps around 0/180 in OpenCV (0-5 or 160-180)
    if ((h <= 5 || h >= 160) && s > 80 && v > 80) return RubiksCube::COLOR::RED;

    // Green: hue 35-85
    if (h >= 35 && h <= 85 && s > 50 && v > 50) return RubiksCube::COLOR::GREEN;

    // Blue: hue 85-130
    if (h >= 85 && h <= 130 && s > 50 && v > 50) return RubiksCube::COLOR::BLUE;

    // Fallback: if high value and low saturation, likely white; otherwise unknown
    if (v > 150 && s < 80) return RubiksCube::COLOR::WHITE;
